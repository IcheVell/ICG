...
public class SceneFileParser {
    ...
    private final Map<FigureType, Integer> pointsMap = new EnumMap<FigureType, Integer>(FigureType.class);

    public SceneFileParser(String path, SceneModel sceneModel) {
        this.sceneModel = sceneModel;

        pointsMap.put(FigureType.SPHERE, 1);
        pointsMap.put(FigureType.BOX, 2);
        pointsMap.put(FigureType.TRIANGLE, 3);
        pointsMap.put(FigureType.QUADRANGLE, 4);
    ...
    public void parseSceneFileHeader() {
        try {
            ...
            for (int i = 0; i < ambients.length; i++) {
                if (ambients[i] < 0 || ambients[i] > 255) {
                    JOptionPane.showMessageDialog(null, "Invalid ambient: " + ambients[i], "Error", JOptionPane.ERROR_MESSAGE);
                    sceneModel.clear();
                    return;
                }
            }
    ...
    public void parseSceneFileSection() {
        try {
            ...
            while (figureType.length != 0) {
                ...
                switch (FigureType.valueOf(figureType[0])) {
                    case SPHERE -> parseSphere();
                    case BOX -> parseBox();
                    case TRIANGLE -> parseTriangle();
                    case QUADRANGLE -> parseQuadrangle();
                }
